...
var alerted = false
var just_alerted = true
func _ready():
	$Sprite.set_frame(int(rand_range(0,16)))
func alert():
	alerted = true
	#$Sprite.visible = true
	
	
	var sound = int(rand_range(0,3))
	if sound == 0:
		var strm = $alert1.stream as AudioStreamOGGVorbis
		strm.set_loop(false)
		$alert1.play()
	elif sound == 1:
		var strm = $alert2.stream as AudioStreamOGGVorbis
		strm.set_loop(false)
		$alert2.play()
	else:
		var strm = $alert3.stream as AudioStreamOGGVorbis
		strm.set_loop(false)
		$alert3.play()
	#print("alerted!")
...
func has_line_of_sight(start_coord, end_coord, tilemap: TileMap):
	#print("start coord:" , start_coord , "end: " , end_coord)
...
func get_grid_path(start_coord, end_coord, astar: AStar2D, astar_points_cache: Dictionary):
	...
	#print([str(start_coord)], astar_points_cache[str(end_coord)])
	return path
